...
Object: Dynamic Allocate Matrix
*/
...
// *
	...
	// function
	void assign(int m, int n, float* c);
...
void Matrix::assign(int m, int n, float* c) {
	M = m;
	N = n;
	ad = new float*[M];
...
Matrix::Matrix(int m, int n, float* c): M(m), N(n) {
	ad = new float*[M];
	for (unsigned int i = 0; i < M; i++) {
		ad[i] = new float[N];
		for (unsigned int j = 0; j < N; j++) {
